...
在[Dubbo 注解驱动](/en/blog/2018/08/07/dubbo-注解驱动/)例子中，无论是服务提供方，还是服务消费方，均需要转配相关配置Bean：
...
在 Dubbo 官方用户手册的[“属性配置”](/en/overview/mannual/java-sdk/reference-manual/config/properties/)章节中，`dubbo.properties` 配置属性能够映射到  `ApplicationConfig` 、`ProtocolConfig` 以及 `RegistryConfig` 的字段。从某种意义上来说，`dubbo.properties`  也是 Dubbo 的外部化配置。
