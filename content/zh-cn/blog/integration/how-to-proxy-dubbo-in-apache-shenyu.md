...
[Apache ShenYu(Incubating)](https://shenyu.apache.org/zh-cn/docs/index) 是一个异步的，高性能的，跨语言的，响应式的 `API` 网关。兼容各种主流框架体系，支持热插拔，用户可以定制化开发，满足用户各种场景的现状和未来需求，经历过大规模场景的锤炼。
...
`shenyu-admin`是`Apache ShenYu`后台管理系统， 启动的方式有多种，本文通过 `[本地部署](https://shenyu.apache.org/zh-cn/docs/deployment/deployment-local)` 的方式启动。启动成功后，需要在基础配置`->`插件管理中，把`dubbo` 插件设置为开启，并设置你的注册地址，请确保注册中心已经开启。
...
>
> 具体的介绍与使用请看: `[选择器与规则管理](https://shenyu.apache.org/zh-cn/docs/user-guide/admin-usage/selector-and-rule)` 。
...
客户端接入的相关配置请参考 `[客户端接入配置](https://shenyu.apache.org/zh-cn/docs/user-guide/register-center-access)` 。
...
数据同步的相关配置请参考 `[数据同步配置](https://shenyu.apache.org/zh-cn/docs/user-guide/use-data-sync)` 。
...
具体的注册逻辑由注册中心实现，请参考 `[客户端接入原理](https://shenyu.apache.org/zh-cn/docs/design/register-center-design/)` 。
