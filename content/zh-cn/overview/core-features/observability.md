...
- Git
- Maven
- Prometheus [安装教程](../../reference/integrations/prometheus.md)
- Grafana [安装教程](../../reference/integrations/grafana)
...
为了使演示结果更清晰，下面是使用浏览器发起的GET请求获取到的指标数据：
...
上面仅仅以单台机器演示，将IP关键词替换为实际IP即可，当然在实际企业应用中可以根据实际情况配置为动态服务发现地址，这样可以获取所有服务的指标数据。
