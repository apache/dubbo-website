...
在泛化调用或跨语言调用场景下，Go 的 variadic 方法签名（例如 `args ...string`）容易引入额外的契约歧义。
现有 variadic 服务仍保持兼容，但对于新的跨语言服务或需要面向泛化调用的服务，不建议继续将 `...T` 作为 RPC 契约的一部分。
...
现有 variadic 服务仍保持兼容，这里的建议主要面向新的服务契约设计。
如果服务需要面向跨语言消费者、网关转发或泛化调用，建议尽量避免将 variadic 参数作为公开 RPC 契约的一部分。
